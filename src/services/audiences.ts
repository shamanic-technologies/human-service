...
function buildLayer1SystemPrompt(): string {
  return [
    ...
    "named target audiences. Be GENEROUS: aim for a RICH set -- ballpark 6-8",
    "audiences as an order of magnitude, and MORE is fine when the request",
    "supports it. A larger, well-partitioned set is better than a few broad",
    "buckets, because each audience maps to its own tighter people-search filter",
    "set and gets contacted with a more relevant angle.",
    "",
    ...
    "boutiques and communication agencies\" = 2 audiences; \"founders and heads",
    ...
    "separately-contactable segments. Each partition MUST be MECE (Mutually",
    "Exclusive -- no overlap -- and Collectively Exhaustive -- covers the whole",
    "space). Use these default partitions when the axis is unspecified and",
    "pertinent:",
    ...
    "    scope; if partly specified, partition MECE within what remains).",
    ...
    "",
    "Reaching the 6-8+ target: pick the axis (or a few axes) that make the most",
    "BUSINESS sense to partition -- geography is usually the strongest first",
    "lever. You do NOT have to take the full cartesian product of every axis",
    "(that yields absurd 100+ tiny buckets); explode generously but sensibly to",
    "reach a rich set. If the caller is explicit about grouping (\"US and Europe",
    "separately\", \"one broad list\"), honor that intent exactly -- a caller who",
    "asks for one broad list gets one audience.",
    "",
    ...
    "THREE filterable axes -- make each audience concrete on ALL the axes that",
    "apply (the caller's stated constraints PLUS the partition value you assigned",
    "it):",
    ...
    "  EVERY constraint the caller stated PLUS the specific partition value you",
    "  assigned this audience (its region / revenue band / size band / funding",
    "  stage / role / company type). State that partition value explicitly and",
    "  never drop a caller-stated constraint.",
    "",
