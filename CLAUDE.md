...
3. **People gateway v1** — a provider-agnostic façade over `apollo-service`
   ...
   (apollo/apify own the paid call, and apollo-service owns email verification);
...
- **No cost declaration here** — apollo/apify own the paid call (and
  apollo-service owns the email verification); human-service only forwards
  `x-run-id`.
...
Apollo's "verified" addresses bounced at **~8%** in September 2026. Getting a
correct, deliverable address is the PROVIDER's job, so **apollo-service verifies
every email it reveals** (BounceVerify: real SMTP + catch-all detection, #267)
and returns `emailVerification: { verdict, deliverable, ... } | null` beside
`person` on `/enrich` and `/match`. apollo-service owns the verifier, its cost
(`apify-bounceverify-email`) and the policy (`deliverable` = verdict `valid`
only; `catch_all`, `invalid`, `unknown`, `risky` are not deliverable).
human-service only ACTS on the answer, in `finalizeResolved`
(`src/lib/email-verification.ts` `readEmailVerification`):

- **Deliverable ⟹ served; not deliverable ⟹ dropped** — and still recorded as a
  serve (bronze + silver), so the suppression layer stops a later request paying
  to reveal them again. Never handed back, never tagged as an audience member.
- **An email with no usable verdict FAILS LOUD** (`EmailVerificationError` →
  502): no "assume deliverable" fallback. apify-service returns no verdict (and
  is not deployed), so an apify reveal fails loud here too.
- **Every verdict is persisted** on `lead_serves.email_verdict` (migration
  `0026`, nullable: NULL = served before the gate, or no address) — the join key
  for measuring the bounce rate per verdict against instantly-service's
  `email_bounced` events.
- **History**: v0.46.3 verified in human-service itself (serving `valid` +
  `catch_all`), v0.46.4 dropped `catch_all`, then verification moved to
  apollo-service on the owner's call ("human-service manages humans"). Measured
  before shipping (100 bounced + 100 delivered prod addresses): invalid 24/3,
  unknown 32/15, catch_all 41/46, valid 3/36 — ~40%, ~15%, ~7%, <1% bounce.
- Guarded by `tests/unit/email-verification.test.ts` (the reader) and
...
**Three stages — layer-1 restate (human-service) → faithful-Apollo EXPLORATION
(apollo-service) → CHOOSE + LABEL (human-service) → persist:** ("one filter
vocabulary" Wave 2 — the in-human-service Layer-2 agentic loop + apolloDslToNeutral
mapper are DELETED; apollo-service owns the NL→faithful-Apollo-filters loop. It
EXPLORES; it no longer DECIDES.)
...
1. **LAYER 1 (provider-agnostic, ONE LLM call via chat-service)** —
   ...
   - **⚠️ `buildLayer1SystemPrompt` is a `[...].join("\n")` array AND both
     ...
     any prompt edit.
2. **APOLLO EXPLORATION (apollo-service owns it)** — `suggestApolloAudience` calls
   ...
   counts) INTERNALLY and returns every round it explored as `candidates[]` —
   each `{ apolloAudienceId, filters (faithful, opaque), count, sample (ten rows
   of employer + title; Apollo's free teaser redacts location), notes (the
   model's own three one-sentence notes from that round) }` — alongside its
   unchanged legacy `apolloAudienceId` / `filters` / `count` / `degraded` top-level
   fields. `candidates` is ADDITIVE on their side, so there is NO deploy-ordering
   constraint: a deploy that sends none yields exactly ONE candidate synthesised
   from the legacy fields (`asApolloAudience` in `src/lib/apollo-audiences.ts`),
   which is the pre-chooser behaviour byte for byte. human-service does NOT build,
   validate, or even understand Apollo's filter vocabulary — it routes + caches the
   opaque result. Best-provider collapse degenerates to apollo (apify is inert).
3. **THE CHOOSER — ONE LLM call that picks the audience AND writes its name +
   description** (`src/services/audience-chooser.ts`,
   `chooseAudienceCandidate`). It receives the caller's ORIGINAL request plus
   EVERY candidate with its count AND its sample rows AND its notes, and answers
   `{ chosen (1-based), why, name, description, degraded }`.
   - **Why the decision lives here.** apollo-service decided for weeks, and every
     mechanism for deciding failed identically: a model grading its OWN proposal
     in isolation answers the same way every time. `reachesOffTarget` /
     `leavesTargetUnreached` came back clean on everything; `matchesRequest` came
     back true on World Health Organization and HORNBACH Baumarkt; `showable` came
     back true on **60 of 60 rounds**, so selection collapsed to plain argmax-count
     and one production run returned 179,156 people at Mars, Lidl, Bucherer and
     Manor. The exploration was never the problem — nearly every run contains a
     round in the low hundreds to ~2,000 with recognisable Swiss drugstores. So the
     decision moved to the service that owns the audience concept and talks to the
     customer, and to a chooser that authored none of the sets. **Choosing among N
     is COMPARATIVE**, which is precisely what does not degenerate the way an
     absolute self-grade does.
   - **The SAMPLES are the whole point.** On counts alone it reproduces the argmax
     that just failed. Side by side, "Mars, Lidl, Bucherer, Manor" against
     "Abderhalden Drogerie AG, Bio Partner Schweiz" is not a hard call. Candidates
     are NEVER hidden from it to simplify the prompt — that would take the
     comparison away from the only step that can make it.
   - **NOTHING SELECTS IN CODE.** No argmax on count, no scoring function, no
     ranking, no tie-break, no count floor, no target band, no minimum size. Code
     only validates that the index the model named exists (out of range ⟹ 502).
   - **The business context is given in BOTH halves.** The audience feeds a COLD
     EMAIL campaign, so a somewhat-too-large audience carrying some noise beats a
     too-narrow one; an engagement is hard to justify below roughly 2,000
     contactable people and a durably successful client is more like 50,000. AND
     those numbers explain why volume MATTERS — they are NOT a floor to reach.
     Some markets are genuinely small (Swiss Drogerien are 2-to-5-person shops
     Apollo does not index in the thousands); **a genuinely small market is a
     valid, correct answer**. Without the second half the chooser picks the round
     full of manufacturers. `tests/unit/audience-chooser-prompt.test.ts` pins both.
   - **ONE call does the choice AND the label, and that is the point, not a
     shortcut.** Until #243 the description was regenerated from the final filters
     by a SECOND call (`generateAudienceDescription`, the #234 relabel). Two calls
     can disagree about what the audience is; one cannot. So the thing that picks
     is the thing that describes, and the relabel step is GONE from `/suggest`
     (`generateAudienceDescription` stays — the historical description backfill in
     `src/routes/backfill.ts` is its remaining caller).
   - **The NAME now comes from the chooser too**, so it describes the set that was
     actually chosen rather than the layer-1 input specification. It remains the
     idempotency key (unique on `(org, brand, offer, lower(name))`), so a
...
- **`degraded` — the CHOOSER's verdict, and a candidate is ALWAYS returned.**
  ...
  onboarding shows the customer something they can judge and reject rather than an
  error screen. The verdict is stored on **`audiences.degraded`** (migration
  ...
  filters or warns on a degraded audience; it is persisted and activatable exactly
  like any other, and the customer decides. A chooser answer **without** the field
  means `false`, never an error and never a value invented here: only an explicit
  `true` degrades. `false` is likewise the truthful value for every row created
  before the concept existed. apollo-service still emits its OWN `degraded` on the
  top-level build; `/suggest` no longer reads it (the chooser sees the same
  candidates apollo graded and answers for itself), but the two other paths that
  build through apollo-service and do NOT choose (`migrateApifyAudienceToApollo`,
  `backfillApolloAudiencePointer`) still persist apollo's verdict verbatim.
...
- **Two cost owners, none here.** Layer 1 (`openai`/`gpt-pro`/`disableThinking:true`,
  ...
  **apollo-service** (which calls chat-service internally). So **human-service
  still declares no cost** — the invariant holds.
